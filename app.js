...
function setNowPlayingLyricLine(element, track, line) {
  if (!element) return;
  if (!line) { element.textContent = ''; element.disabled = true; element.onclick = null; return; }
  element.textContent = line.text; element.disabled = false;
...
async function restorePreferences() {
  ...
  if (record) {
    ...
    if (['off', 'single', 'focus'].includes(record.lyricsDisplay)) preferences.lyricsDisplay = record.lyricsDisplay;
  }
  applyPreferences();
}
...
function renderLyrics(track = tracks.find((entry) => entry.id === activeLyricsId)) {
  ...
  if (track.syncedLyrics?.length) {
    ...
    content.querySelectorAll('[data-lyric-time]').forEach((line) => {
      line.onclick = () => {
        const lyric = track.syncedLyrics[Number(line.dataset.lyricIndex)];
...
function syncNowPlaying(track = tracks.find((entry) => entry.id === currentId)) {
  ...
  if (changed) {
    // Clear the outgoing lyric before the coordinated visual swap starts. The next
    // line is rendered only after this track becomes the displayed Now Playing item.
    resetNowPlayingLyrics(track);
...
function closeNowPlaying() {
  ...
  setLyricsFocusMode(false); setVisualMinimalMode(false);
...
function updatePendingImportCard() {
  ...
  card.innerHTML = `<div class="pending-import-icon">↓</div><div class="pending-import-copy"><strong>${stale ? 'Still want to finish this import?' : 'Finish your import'}</strong><small>Downloaded the audio? Choose the file to add it to Zombie. · ${pendingImportAge(pending)}</small></div><button id="finishPendingImport" class="pending-import-primary">Choose Downloaded Audio</button><details class="pending-import-more"><summary>More</summary><button id="reopenPendingConverter">Open converter (optional)</button><button id="restartPendingImport">Start Over</button><button id="dismissPendingImport">Not now</button><button id="cancelPendingImport">Cancel</button></details>`;
  $('#finishPendingImport').onclick = () => openFinishImportStep(pendingSource(pending), { returning: true });
  $('#reopenPendingConverter').onclick = () => launchConverter(pendingSource(pending));
...
function openLinkImporter(value = linkImportContext?.url || pendingLinkImport?.sourceUrl || '') {
  ...
  $('#sheetContent').innerHTML = `${importSteps('link')}<section class="link-import-intro"><i>↗</i><span><strong>Paste your link</strong><small>For video links, save the audio to Files first. Direct audio can be imported here.</small></span></section><label class="link-import-field">YouTube or audio link<span class="link-url-wrap"><input id="linkImportUrl" type="url" inputmode="url" autocomplete="off" autocapitalize="none" spellcheck="false" placeholder="https://…" value="${escapeHTML(value)}"><button id="clearLinkImportUrl" class="link-clear-button" aria-label="Clear link">×</button></span></label><p id="linkImportFeedback" class="link-import-feedback" role="status"></p><div class="link-import-actions"><button id="pasteLinkImport" class="sheet-option">Paste from Clipboard</button><button id="continueLinkImport" class="link-import-continue">Continue</button></div>${pendingLinkImport ? '<button id="startOverLinkImport" class="link-import-text-action">Start Over</button>' : ''}<p class="sheet-note import-privacy-note">Your music is saved locally after you choose the downloaded audio file.</p>`;
...
function openConverterFallback(source) {
  $('#sheetTitle').textContent = 'Open in Safari did not work';
  $('#sheetContent').innerHTML = `${importSteps('convert')}<p class="sheet-note">Your link is saved. Safari or a Home Screen app can block a new converter tab. Download the audio elsewhere, then choose it from Files—Zombie will import it normally.</p><button id="chooseFallbackAudio" class="link-import-continue">Choose Downloaded Audio</button><button id="copyLinkAgain" class="sheet-option">Copy link again</button><button id="backToLinkImport" class="link-import-text-action">Back to link</button>`;
  $('#chooseFallbackAudio').onclick = () => openFinishImportStep(source, { returning: true });
  ...
  $('#backToLinkImport').onclick = () => openLinkImporter(source.url);
  showSheet();
}
...
function openConverterStep(source, { returning = false } = {}) {
  if (returning || (pendingLinkImport?.sourceUrl === source.url && pendingLinkImport?.stage === 'converter-opened')) { openFinishImportStep(source, { returning: true }); return; }
  $('#sheetTitle').textContent = 'Get audio into Files';
  $('#sheetContent').innerHTML = `${importSteps('convert')}<section class="link-import-intro"><i>↓</i><span><strong>Import the downloaded audio</strong><small>Zombie adds the audio after you choose it from Files.</small></span></section><p class="converter-short-instructions">For the most reliable iPhone experience, download or save audio you own to <strong>Files</strong>, then return here.</p><button id="finishWithoutOpeningConverter" class="link-import-continue">Choose Downloaded Audio</button><button id="openLinkConverter" class="sheet-option">Open converter (optional)</button><button id="copyLinkForOptionalConverter" class="link-import-text-action">Copy link</button><p class="sheet-note import-privacy-note">Some websites cannot open or download inside a Home Screen app. Your link stays saved either way.</p>`;
  ...
  $('#copyLinkForOptionalConverter').onclick = () => { void copyLinkForConverter(source).then(() => toast('Link copied ✓')).catch(() => toast('Copy the link from Zombie if needed')); };
  showSheet();
}
...
function findPossibleDuplicate(file, metadata, source) {
  ...
  return tracks.find((track) => track.fingerprint === fingerprint || (source?.url && track.sourceUrl === source.url) || (source?.videoId && track.sourceVideoId === source.videoId) || (fileName && String(track.fileName || '').trim().toLocaleLowerCase() === fileName && Number(track.size) > 0 && Number(track.size) === Number(file.size)));
}
